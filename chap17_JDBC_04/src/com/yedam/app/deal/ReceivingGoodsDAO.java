...
//reveiving과 takeout goods 테이블은 primarykey가 없어서 수정과 삭제가 안됨.
public class ReceivingGoodsDAO extends DAO {
	...
	public void insert(DealInfo info) {
		try {
			connect();
			String sql = "INSERT ITNO receiving_goods (product_id, product_amount) VALUES (?, ?)";
	...
	public List<DealInfo> selectAll(){
		...
		try {
			connect();
			String sql = "SELECT r.deal_date, r.product_id, p.product_name, r.product_amount FROM products p JOIN receiving_goods r ON p.product_id = r.product_id ORDER BY r.deal_date";
			pstmt = conn.prepareStatement(sql);
	...
	public List<DealInfo> selectAll(Date dealDate){
		...
		try {
			connect();
			String sql = "SELECT r.deal_date, r.product_id, p.product_name, r.product_amount FROM products p JOIN receiving_goods r ON p.product_id = r.product_id WHERE deal_date =? ORDER BY r.deal_date";
			pstmt = conn.prepareStatement(sql);
	...
	public List<DealInfo> selectAll(int productId){
		...
		try {
			connect();
			String sql = "SELECT r.deal_date, r.product_id, p.product_name, r.product_amount FROM products p JOIN receiving_goods r ON p.product_id = r.product_id WHERE product_id =? ORDER BY r.deal_date";
			pstmt = conn.prepareStatement(sql);
